...
function Ventes() {
  ...
  const updateQuantite = (id, q) => {
    setCart(cart.map(item => {
      ...
      const med = medicaments.find(m => m.id === id);
      const maxStock = med ? med.quantite : 0;
      ...
      return item.id === id ? { ...item, quantite: newQ } : item;
    ...
    const validerVente = () => {
      ...
      for (let item of cart) {
        ...
        if (item.quantite > med.quantite) {
          setMessage(`Stock insuffisant pour ${med.nom}`);
      ...
      axios.post("http://localhost:8000/api/ventes", {
        ...
        .then(() => {
          ...
          fetchMedicaments();
          fetchVentes();
          setSuccess("Vente validée avec succès ✅");
  ...
  if (user.role !== "pharmacien" && user.role !== "admin") {
    return <h2 className="p-5">Access Denied</h2>;
  }
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {medicaments
              ...
              <div key={m.id} className="border p-4 rounded-xl hover:shadow transition">
                ...
                >
                  {getRemainingStock(m) <= 0 ? "Stock épuisé" : "Ajouter"}
                </button>
